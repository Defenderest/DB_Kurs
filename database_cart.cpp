...
bool DatabaseManager::addOrUpdateCartItem(int customerId, int bookId, int quantity)
{
    ...
    // Використовуємо INSERT ... ON CONFLICT ... UPDATE для атомарного додавання/оновлення
    query.prepare(R"(
        INSERT INTO cart_item (customer_id, book_id, quantity)
    ...
    qInfo() << "Товар (bookId" << bookId << ", quantity" << quantity << ") успішно додано/оновлено в корзині для customerId" << customerId;
    return true;
}
...
bool DatabaseManager::removeCartItem(int customerId, int bookId)
{
    ...
    if (!query.exec()) {
        qCritical() << "Помилка при видаленні товару (bookId" << bookId << ") з корзини для customerId" << customerId << ":" << query.lastError().text();
        return false;
    }

    if (query.numRowsAffected() > 0) {
        qInfo() << "Товар (bookId" << bookId << ") успішно видалено з корзини для customerId" << customerId;
    } else {
        qWarning() << "removeCartItem: Товар (bookId" << bookId << ") не знайдено в корзині для customerId" << customerId << " (можливо, вже видалено).";
    }
    return true; // Повертаємо true, навіть якщо нічого не було видалено (операція пройшла без помилок БД)
}
...
bool DatabaseManager::clearCart(int customerId)
{
    ...
    if (!query.exec()) {
        qCritical() << "Помилка при очищенні корзини для customerId" << customerId << ":" << query.lastError().text();
        return false;
    }

    qInfo() << "Корзину успішно очищено для customerId" << customerId << "(видалено рядків:" << query.numRowsAffected() << ")";
    return true;
}
